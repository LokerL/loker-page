import { navbar } from "vuepress-theme-hope";

export default navbar([
  "/",
  "/home",
  { text: "使用指南", icon: "creative", link: "/guide/" },
  {
    text: "加入Q群",
    icon: "group",
    children: [
      {
        text: "tts-vue反馈群⑤(439382846)-满",
        icon: "group",
        link: "http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=aFtHnpeEerVM7UCV8NTSeqsQrUPgo0G2&authKey=g1KT%2F7xaoa%2FT3usmwk5YRIIdkzeJ4bW6tsWCgcopbzCRTwseXTGQS54ZDTmRPmt6&noverify=0&group_code=439382846",
      },
      {
        text: "tts-vue反馈群④(781659118)-已满",
        icon: "group",
        link: "http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=AtvxF5GXuLzd8Y9lhPcEnsOddAUO_2Db&authKey=amRtcf3%2FiYqERTYl7gPxXnaPCaKc8CXmewlQKnWoSdxjk07xuLMG1JKvRI7mLysG&noverify=0&group_code=781659118",
      },
      {
        text: "tts-vue反馈群③(474128303)-已满",
        icon: "group",
        link: "http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=7gvofti_Vl8hyDtZqVxuAwQSyYOaODOT&authKey=WBGICfV9oJXjeXL01KGcxg6FFzgyLgejgle0ZFRQpHMWDha4HftPvqfBGojiSHko&noverify=0&group_code=474128303",
      },
      {
        text: "tts-vue反馈群②(702034846)-已满",
        icon: "group",
        link: "http://qm.qq.com/cgi-bin/qm/qr?_wv=1027&k=5-vK4VI8ueWoF-cBDl6PjBzfDeWRqmvw&authKey=ZHIxqKeYEGewEZFtKtT5o1UNbTtAEnMOe4vH2haBIxA8edc1U%2F4rn5R9RKSv5%2BRF&noverify=0&group_code=702034846",
      },
      {
        text: "tts-vue反馈群①(752801820)-已满",
        icon: "group",
        link: "https://jq.qq.com/?_wv=1027&k=bMNZfGxs",
      },
    ],
  },
]);
